...
def calc_N_values(lengths,expectedTotalSize):
    if expectedTotalSize ==1:
        expectedTotalSize=sum(lengths)
...
def get_len_stats(lengths,expectedTotalSize):
    res = {}
...
def main():
 if __name__ == "__main__":
    if len(argv) == 1: # print usage
        print "Usage: fasta_contents_simple.py fastaFile expectedTotalSize"
        exit()
    KEYS = ['numseq', 'numbps','avglen','stdlen','minlen','maxlen','medlen','n25idx','n25len','n50idx','n50len','ng50idx','ng50len','n75idx','n75len','n95idx','n95len','counts']
    ...
    if len(argv) == 3: # expexted size is given
         expectedTotalSize=int(argv[2])
    else:
         expectedTotalSize=1
    for fna_file in fileName:
        file_storage.append(fna_file)
    print "Statistics for file:"+str(fna_file)
    ...
    for fn, lengths, counts in zip(file_storage, len_storage, counts_storage):
        len_stats = get_len_stats(lengths,expectedTotalSize)
        N_vals = calc_N_values(lengths,expectedTotalSize)
        result[fn] = {}
    ...
    print 'NG50 sequence index:\t%s'\
        %('\t'.join(["%s"%(result[fn]['NG50idx']) for fn in file_storage]))
    print 'NG50 sequence length:\t%s'\
        %('\t'.join(["%s"%(result[fn]['NG50len']) for fn in file_storage]))
    ...
    # for fn in file_storage:
        ...
        # count_tgac = result[fn]['counts']['G'] + result[fn]['counts']['C'] +\
            # result[fn]['counts']['A'] + result[fn]['counts']['T']
        # perc_gc = (count_gc/count_tgac)*100
